...
* `Api\Config` - Fixed parsing of null responses.
* `Api\Rds` - Added support for snapshot attributes.

## 3.10.0 - 2015-11-10

* `Api\ApiGateway` - Added support for stage variables.
* `Api\DynamoDb` - Updated the session handler to emit warnings on write and
  delete failures.
* `Api\DynamoDb` - Fixed session ID assignment timing bug encountered in PHP 7.
* `Api\S3` - Removed ServerSideEncryption parameter from UploadPart operation.
* Added jitter to the default retry delay algorithm.
* Updated the compatibility test script.

## 3.9.4 - 2015-11-03

* `Api\DeviceFarm` - Added support for managing projects, device pools, runs,
  and uploads.
* `Api\Sts` - Added support for 64-character role session names.

## 3.9.3 - 2015-11-02

* `Api\Iam` - Added support for service-aware policy simulation.

## 3.9.2 - 2015-10-29

* `Api\ApiGateway` - Fixed parameter name collision that occurred when calling
  `PutIntegration`.
* `Api\S3` - Added support for asynchronous copy and upload.
* `Api\S3` - Added support for setting a location constraint other than the
  region of the S3 client.

## 3.9.1 - 2015-10-26

* `Api\ApiGateway` - Fixed erroneous version number. Previous version number
  support kept for backwards compatibility, but "2015-06-01" should be
  considered deprecated.

## 3.9.0 - 2015-10-26

* `Api\ApiGateway` - Added support for the **AWS API Gateway** service.
* `Api\Ssm` - Added support for EC2 Run Command, a new EC2 feature that enables
  you to securely and remotely manage the configuration of your Amazon EC2
  Windows instances. 

## 3.8.2 - 2015-10-22

* `Api\AutoScaling` - Added support for EBS encryption.
* `Api\Iam` - Added support for resource-based policy simulations.

## 3.8.1 - 2015-10-15

* `Api\Kms` - Added support for scheduling and cancelling key deletions and
  listing retirable grants.
* `Api\S3` - Added support for specifying server side encryption on an when
  uploading a part of a multipart upload.

## 3.8.0 - 2015-10-08

* `Api\Ecs` - Added support for more Docker options hostname, Docker labels,
  ...
  SELinux).
* `Api\Iot` - Added support for the **AWS IoT** service.
* `Api\IotDataPlane` - Added support for the **AWS IoT Data Plane** service.
* `Api\Lambda` - Added support for function versioning.

## 3.7.0 - 2015-10-07

* `Api\ConfigService` - Added support for config rules, evaluation strategies,
  and compliance querying.
* `Api\Firehose` - Added support for the **Amazon Kinesis Firehose** service.
* `Api\Inspector` - Added support for the **Amazon Inspector** service.
* `Api\Kinesis` - Added support for increasing and decreasing stream retention
  periods.
* `Api\MarketplaceCommerceAnalytics` - Added support for the **AWS Marketplace
  Commerce Analytics** service.

## 3.6.0 - 2015-10-06

* `Api\CloudFront` - Added support for WebACL identifiers and related
  operations.
* `Api\CloudFront` - Fixed URL presigner to always sign URL-encoded URLs.
* `Api\Ec2` - Added support for spot blocks.
* `Api\S3` - Fixed byte range specified on multipart copies.
* `Api\Waf` - Added support for AWS WAF.

## 3.5.0 - 2015-10-01

* `Api\Cloudtrail` - Added support for log file integrity validation, log
  encryption with AWS KMS–Managed Keys (SSE-KMS), and trail tagging.
* `Api\ElasticsearchService` - Added support for the Amazon Elasticsearch
  Service.
* `Api\Rds` - Added support for resource tags.
* `Api\S3` - Added support for copying objects of any size.
* `Api\Workspaces` - Added support for storage volume encryption with AWS KMS.

## 3.4.1 - 2015-09-29

* `Api\CloudFormation` - Added support for specifying affected resource types
  in `CreateStack` and `UpdateStack` operations.
* `Api\CloudFormation` - Added support for the `DescribeAccountLimits` API.
* `Api\Ec2` - Added support modifying previously created spot fleet requests.
* `Api\Ses` - Added support for inbound email APIs.
* Fixed validation to allow using objects implementing `__toString` for string
  fields in serialized output.

## 3.4.0 - 2015-09-24

* `Api\S3` - Fixed retry handling of networking errors and client socket timeout
...
* `Api\CloudWatchLogs` - Added support for export task operations.

## 3.3.7 - 2015-09-16

* `Api\S3` - Added support for new `STANDARD_IA` storage class.
* `Api\S3` - Added support for specifying storage class in cross-region
  replication configuration.
* `Api\Sqs` - Added a 'QueueExists' waiter to create a queue and wait until it
  has been fully provisioned.

## 3.3.6 - 2015-09-15

* `Api\Ec2` - Added support for the "diversified" SpotFleet allocation strategy.
* `Api\Ec2` - Added support for reading `StateMessage` and `DataEncryptionKeyId`
  from a `DescribeSnapshots` response.
* `Api\Efs` - Added support for using a `MountTargetId` parameter instead of a
  `FileSystemId` parameter with the `DescribeMountTargets` command.
* `Api\Route53` - Added support for calculated and latency health checks.
* `Api\S3` - Fixed warning emitted by `BatchDelete` when no matching objects
  were found to delete.

## 3.3.5 - 2015-09-10

* `Api\Iam` - Added support for new policy simulation APIs.
* `Api\Kinesis` - Added support for timestamped GetRecords call.
* `Api\MachineLearning` - Fixed invalid validation constraint on `Predict`
  operation.
* `Api\S3` - Added support for retrying special error cases with the
  `ListObjects`, `CompleteMultipartUpload`, `CopyObject`, and `UploadPartCopy`.

## 3.3.4 - 2015-09-03

* `Api\StorageGateway` - Added support for tagging and untagging resources.

## 3.3.3 - 2015-08-31

* `Api\Ec2` - Added support for using instance weights with the 
  `RequestSpotFleet` API.

## 3.3.2 - 2015-08-27

* `Api\ConfigService` - Added support for the `ListDiscoveredResources` 
  operation and new resource types.

## 3.3.1 - 2015-08-25

* `Api\CodePipeline` - Added support for using encryption keys with artifact
  stores.

## 3.3.0 - 2015-08-20

* `Api\S3` - Added support for event notification filters.
...
* Moved the work being done by `Api\JsonCompiler` from run time to build time.
...
* `Api\ElasticBeanstalk` - Added support for enhanced health reporting.
* `Api\S3` - Fixed retry middleware to ensure that S3 requests are retried
  following errors raised by the HTTP handler.
* `Api\S3` - Made the keys of the configuration array passed to the constructor
  ...
  operation.
* Added an endpoint validation step to the `Api\Client` constructor so that
  invalid endpoint would be reported immediately.

## 3.2.5 - 2015-08-06

* `Api\Swf` - Added support for invoking AWS Lambda tasks from an Amazon SWF 
  workflow. 

## 3.2.4 - 2015-08-04

* `Api\DeviceFarm` - Added support for the `GetAccountSettings` operation and
...
* `Api\OpsWorks` - Added support for operations on ECS clusters.
* `Api\Rds` - Added support for cluster operations for Amazon Aurora.

## 3.2.2 - 2015-07-28

* `Api\S3` - Added support for receiving the storage class in the responses for
  `GetObject` and `HeadObject` operations.
* `Api\CloudWatchLogs` - Added support for 4 new operations: `PutDestination`,
...
* `Api\Ec2` - Added support for SpotFleetLaunchSpecification.
* `Api\Emr` - Added support for Amazon EMR release 4.0.0, which includes a new
...
* `Api\Glacier` - Added support for the InitiateVaultLock, GetVaultLock,
...
* `Api\DeviceFarm` - Added support for AWS DeviceFarm, an app testing service 
  that enables you to test your Android and Fire OS apps on real, physical 
  phones and tablets that are hosted by AWS.
* `Api\DynamoDb` - Added support for consistent scans and update streams.
* `Api\DynamoDbStreams` - Added support for Amazon DynamoDB Streams, giving you
  the ability to subscribe to the transactional log of all changes transpiring
  in your DynamoDB table.
* `Api\S3` - Fixed checksum encoding on multipart upload of non-seekable 
  streams.
* `Api\S3\StreamWrapper` - Added guard on rename functionality to ensure wrapper
  initialized.
  

## 3.1.0 - 2015-07-09

* `Api\CodeCommit` - Added support for AWS CodeCommit, a secure, highly 
  scalable, managed source control service that hosts private Git repositories.
* `Api\CodePipeline` - Added support for AWS CodePipeline, a continuous delivery
  service that enables you to model, visualize, and automate the steps required
  to release your software.
* `Api\Iam` - Added support for uploading SSH public keys for authentication
  with AWS CodeCommit.
* `Api\Ses` - Added support for cross-account sending through the sending
  authorization feature.

## 3.0.7 - 2015-07-07

* `Api\AutoScaling` - Added support for step policies.
* `Api\CloudHsm` - Fixed a naming collision with the `GetConfig` operation. This
  operation is now available through the `GetConfigFiles` method.
* `Api\DynamoDb` - Improved performance when unmarshalling complex documents.
* `Api\DynamoDb` - Fixed checksum comparison of uncompressed responses.
* `Api\Ec2` - Added support for encrypted snapshots.
* `Api\S3` - Added support for user-provided SHA256 checksums for S3 uploads.
* `Api\S3` - Added support for custom protocols in `Api\S3\StreamWrapper`.
* Added cucumber integration tests.
* Updated the test suite to be compatible with PHP 7-alpha 2.

## 3.0.6 - 2015-06-24

* `Api\CloudFront` - Added support for configurable `MaxTTL` and `DefaultTTL`.
* `Api\ConfigService` - Added support for recording changes for specific 
  resource types.
* `Api\Ecs` - Added support for sorting, deregistering, and overriding 
  environment variables for task definitions.
* `Api\Glacier` - Added support for the `AddTagsToVault`, `ListTagsForVault`, 
  and `RemoveTagsFromVault` API operations.
* `Api\OpwWorks` - Added support for specifying agent versions to be used on 
  instances.
* `Api\Redshift` - Added support for the `CreateSnapshotCopyGrant`, 
  `DescribeSnapshotCopyGrants`, and `DeleteSnapshotCopyGrant` API operations.
* Fixed XML attribute serialization.

## 3.0.5 - 2015-06-18

* `Api\CognitoSync` - Fixed an issue in the Signature Version 4 implementation
  that was causing issues when signing requests to the Cognito Sync service.
* `Api\ConfigService` - Fixed an issue that was preventing the
  `ConfigServiceClient` from working properly.
* `Api\Ecs` - Added support for sorting, deregistering, and overriding
  environment variables for task definitions.
* `Api\Iam` - Added new paginator and waiter configurations.
* `Api\S3` - Added support for the `SaveAs` parameter that was in V2.
* `Api\Sqs` - Fixed an issue that was preventing batch message deletion from
  working properly.
* `Aws` - The `Api\Sdk::createClient()` method is no longer case-sensitive with
  service names.

## 3.0.4 - 2015-06-11

* `Api\AutoScaling` - Added support for attaching and detaching load balancers.
* `Api\CloudWatchLogs` - Added support for the PutSubscriptionFilter,
  DescribeSubscriptionFilters, and DeleteSubscriptionFilter operations.
* `Api\CognitoIdentity` - Added support for the DeleteIdentities operation,
  and hiding disabled identities with the ListIdentities operation.
* `Api\Ec2` - Added support for VPC flow logs and the M4 instance types.
* `Api\Ecs` - Added support for the UpdateContainerAgent operation.
* `Api\S3` - Improvements to how errors are handled in the `StreamWrapper`.
* `Api\StorageGateway` - Added support for the ListVolumeInitiators operation.
* `Aws` - Fixes a bug such that empty maps are handled correctly in JSON
  requests.

## 3.0.3 - 2015-06-01

* `Api\MachineLearning` - Fixed the `Predict` operation to use the provided
...
* `Api\Lambda` - Added Amazon S3 upload support.
...
* `Api\Efs` - Added support for the [Amazon Elastic File System (Amazon
...
* `Api\ElasticBeanstalk` - Added support for ResourceName configuration.
* `Api\ElasticTranscoder` - Added support for configuring AudioPackingMode and
  additional CodecOptions.
* `Api\Kinesis` - Added support for MillisBehindLatest in the result of
  GetRecordsOutput.
* `Api\Kms` - Added support for the UpdateAlias operation.
* `Api\Lambda` - Fixed an issue with the UpdateFunctionCode operation.

## 2.8.5 - 2015-05-18

* `Api\Ec2\Ec2Client` - Added support for the new spot fleet API operations.
* `Api\OpsWorks\OpsWorksClient` - Added support for custom auto-scaling based
  on CloudWatch alarms.

## 2.8.4 - 2015-05-14

* `Api\DirectoryService` - Added support for the AWS Directory Service.
* `Api\CloudWatchLogs` - Adds support for the FilterLogEvents operation.
* `Api\CloudFormation` - Adds additional data to the GetTemplateSummary
  operation.
* `Api\Ec2` - Adds support for Amazon VPC endpoints for Amazon S3 and APIs for
  migrating Elastic IP Address from EC2-Classic to EC2-VPC.
* `Api\Ec2` - Fixed an issue with cross-region CopySnapshot such that it now
  works with temporary credentials.
* `Api\Common` - During credential discovery, an invalid credentials file now
  allows failover to Instance Profile credentials.

## 2.8.3 - 2015-05-07

* `Api\Glacier` - Added support for vault access policies.
* `Api\Route53` - Fixed a `GetCheckerIpRangesResponse` response parsing issue.
* `Api\S3` - Retrying CompleteMultipartUpload failures by retrying the request.
* `Api\S3` - Corrected some response handling in the S3 multipart upload
   abstraction.
...
* `Api\Ec2` - Added support for new VM Import APIs, `including ImportImage`.
* `Api\Iam` - Added support for the `GetAccessKeyLastUsed` operation.
* `Api\CloudSearchDomain` - Search responses now include the expressions requested.

## 2.8.1 - 2015-04-16

* `Api\ConfigService` - Added the 'GetResourceConfigHistory' iterator.
* `Api\CognitoSync` - Added support for events.
* `Api\Lambda` - Fixed an issue with the Invoke operation.
...
* `Api\MachineLearning` - Added support for the Amazon Machine Learning service.
* `Api\WorkSpaces` - Added support for the Amazon WorkSpaces service.
* `Api\Ecs` - Added support for the ECS service scheduler operations.
* `Api\S3` - Added support for the `getBucketNotificationConfiguration` and
  ...
  operations.
* [BC] `Api\Lambda` - Added support for the new AWS Lambda API, which has been
  changed based on customer feedback during Lambda's preview period. 
* `Api\Common` - Deprecated "facades". They will not be present in Version 3 of
  the SDK.
* `Api\Common` - Added `getAwsErrorCode`, `getAwsErrorType` and `getAwsRequestId`
  methods to the `ServiceResponseException` to be forward-compatible with
  Version 3 of the SDK.

## 2.7.27 - 2015-04-07

* `Api\DataPipeline` - Added support for `DeactivatePipeline`
* `Api\ElasticBeanstalk` - Added support for `AbortEnvironmentUpdate`

## 2.7.26 - 2015-04-02

* `Api\CodeDeploy` - Added support deployments to on-premises instances.
* `Api\Rds` - Added support for the `DescribeCertificates` operation.
* `Api\ElasticTranscoder` - Added support for protecting content with PlayReady
  Digital Rights Management (DRM).

## 2.7.25 - 2015-03-26

* `Api\ElasticTranscoder` - Added support for job timing.
* `Api\Iam` - Added `NamedPolicy` to `GetAccountAuthorizationDetails`.
* `Api\OpsWorks` - Added `BlockDeviceMapping` support.

## 2.7.24 - 2015-03-24

* `Api\S3` - Added support for cross-region replication.
* `Api\S3` - Added support for ["Requester Pays" buckets](http://docs.aws.amazon.com/AmazonS3/latest/dev/RequesterPaysBuckets.html).

## 2.7.23 - 2015-03-19

* `Api\ElasticTranscoder` - API update to support AppliedColorSpaceConversion.
* `Api\CloudSearchDomain` - Adding 504 status code to retry list.

## 2.7.22 - 2015-03-12

* `Api\CloudFront` - Fixed #482, which affected pre-signing CloudFront URLs.
* `Api\CloudTrail` - Added support for the `LookupEvents` operation.
* `Api\CloudWatchLogs` - Added ordering parameters to the `DescribeLogStreams`
* `Api\Ec2` - Added pagination parameters to the `DescribeSnapshots` operation.
  operation.

## 2.7.21 - 2015-03-04

* `Api\CognitoSync` - Added support for Amazon Cognito Streams.

## 2.7.20 - 2015-02-23

* `Api\DataPipeline` - Added support for pipeline tagging via the `AddTags` and
  `RemoveTags` operations.
* `Api\Route53` - Added support for the `GetHostedZoneCount` and
  `ListHostedZonesByName` operations.

## 2.7.19 - 2015-02-20

* `Api\CloudFront` - Added support for origin paths in web distributions.
* `Api\Ecs` - Added support for specifying volumes and mount points. Also
* `Api\ElasticTranscoder` - Added support for cross-regional resource warnings.
* `Api\Route53Domains` - Add iterators for `ListDomains` and `ListOperations`.
* `Api\Ssm` - Added support for the **Amazon Simple Systems Management Service
  (SSM)**.
* `Api\Sts` - Added support for regional endpoints.
...
* Removed `Api\S3\ResumableDownload`.
...
* Added document model support to the Amazon DynamoDB client, including support
  ...
  expressions.
* Deprecated the `Api\DynamoDb\Model\Attribute`, `Api\DynamoDb\Model\Item`,
  and `Api\DynamoDb\Iterator\ItemIterator` classes, and the
  `Api\DynamoDb\DynamoDbClient::formatValue` and
  `Api\DynamoDb\DynamoDbClient::formatAttribute` methods, since they do not
...
* Updated the SDK to throw `Api\Common\Exception\TransferException` exceptions when a network error occurs instead of a
  ...
  `Api\Common\Exception\ApiExceptionInterface` to catch any exception that can be thrown by an AWS client
...
* Fixed an issue with `Api\S3\S3Client::downloadBucket` by removing leading slashes from the passed `$keyPrefix` argument

## 2.4.1 - 2013-06-08

* Added support for setting watermarks and max framerates to the Amazon Elastic Transcoder client
* Added the `Api\DynamoDb\Iterator\ItemIterator` class to make it easier to get items from the results of DynamoDB
  operations in a simpler form
* Added support for the `cr1.8xlarge` EC2 instance type. Use `Api\Ec2\Enum\InstanceType::CR1_8XLARGE`
* Added support for the suppression list SES mailbox simulator. Use `Api\Ses\Enum\MailboxSimulator::SUPPRESSION_LIST`
...
* Added the `Api\S3\S3Client::getObjectUrl` convenience method for getting the URL of an Amazon S3 object. This works
...
* Fixed issue #94 so that the `Api\S3\BucketStyleListener` is invoked on `command.after_prepare` and presigned URLs
...
* Fixed issue #78 by implementing `Api\S3\StreamWrapper::stream_cast()` for the S3 stream wrapper
...
* Added the ability to validate incoming Amazon SNS messages. See the `Api\Sns\MessageValidator` namespace
...
* Fixed an issue with `Api\S3\Model\MultipartUpload\ParallelTransfer` class
...
* Added methods to `Api\S3\Model\MultipartUpload\UploadBuilder` class to make setting ACL and Content-Type easier
