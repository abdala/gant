...
class CommandPool implements PromisorInterface
{
     ...
     *   The function is provided an ApiException object, id of the iterator that
     *   the exception came from, and the aggregate promise that can be
     *   resolved/rejected if you need to short-circuit the pool.
     *
     * @param ClientInterface $client   Client used to execute commands.
            ...
            foreach ($commands as $key => $command) {
                if (!($command instanceof CommandInterface)) {
                    throw new \InvalidArgumentException('Each value yielded by '
                        . 'the iterator must be an Api\CommandInterface.');
                }
                if ($before) {
                    $before($command, $key);
                }
     ...
     *
     * @return array
     * @see \Api\CommandPool::__construct for available configuration options.
     */
